...
(() => {
  ...
  // D43 — the ENGINE's own spelling of a native function's source. `markNative`
  ...
  // (DECISIONS.md D46; the regression it closes is in D47's closing paragraph).
   ...
   * stands down. Each entry `{ target, prop, desc[, remove] }` is held STRONGLY only by a
  ...
  function remember(target, prop, desc, remove) {
    // `remove` (D46): the browser never had this property, so restoring means
    // DELETING it rather than redefining a descriptor that does not exist.
    const entry = remove ? { target, prop, desc, remove: true } : { target, prop, desc };
  ...
  /**
   * ── The spelling of `[native code]` is the ENGINE's, never ours (D43) ──────
   *
  ...
  function restoreAll() {
    for (let i = RESTORES.length - 1; i >= 0; i--) {
        ...
        try {
          ...
          if (e.remove) delete e.target[e.prop];
          else objDefineProperty(e.target, e.prop, e.desc);
        } catch (_) {}
    }
    RESTORES.length = 0;
  }
  ...
  const GPC_REALMS = [];                 // WeakRef<entry>, insertion order — never a strong list of realms (D47)
  const GPC_ENTRY = new RawWeakMap();    // target → entry: held exactly as long as that realm's prototype
  ...
  function installGpc(win) {
    ...
    remember(P, GPC_PROP, d, !d);                       // through the D47 ledger, never a bare push
    ...
    const gpcEntry = { target: P, nativeDesc: d || null, nativeValue, handedBack: false };
    wmSet(GPC_ENTRY, P, gpcEntry);
    pushOwn(GPC_REALMS, new RawWeakRef(gpcEntry));
  }
  ...
  function setGpcSignal(on) {
    ...
    for (let i = 0; i < GPC_REALMS.length; i++) {
      const r = wrDeref(GPC_REALMS[i]);
      if (!r) continue;                                 // that realm is gone
