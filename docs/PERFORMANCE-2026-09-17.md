...
is on it, once per **same-origin** child frame per insertion; a cross-origin frame is remembered
after its first throw and costs one WeakSet lookup.
...
## Quiet page, same instrument, both builds

`?only=none` (no suite runs), then a twin of `paired()` — `a` = live `appendChild`, `b` = the
...
| 3 | 3.37 | 5.86 | +2.49 | 3.44 | 5.48 | +2.04 | −0.10 |
| 5 | 3.86 | 8.15 | +4.28 | 3.65 | 7.50 | +3.86 | −0.03 |
...
Five `127.0.0.1` child frames on a `localhost` page (all five throw `SecurityError` on `document`),
the same paired twin, 2.5 s budget:
...
|---|---|---|---|---|---|
| `appendChild` a plain div — 0 child frames | 2.28 µs | 3.26 | **+0.98** | 1.36× | +0.49 |
| `insertBefore` a plain div — 0 child frames | 3.08 | 3.71 | **+0.63** | 1.31× | +0.73 |
| `innerHTML` = 50-node string — 0 child frames | 128.6 µs | 128.6 | **0** | 1.05× | 0 |
| `appendChild` a plain div — **3 child frames** | 2.49 | 5.57 | **+3.08** | 2.07× | +2.10 |

Calibration this session: `?shim=off` cells 1.000 / 1.000 / 1.004 / 0.955, whole-run
`nullWorstDeviation` 0.123, and the in-page null control (`Element.clientWidth`) on the shim-on run
**0.929** — a worse instrument than D35's day (1.014). Read the 3-frame cell (+1 µs over D35's, i.e.
≈0.3 µs per frame, matching the slope above) and treat the 0-frame differences as noise. The D35
build's standard cells were not re-taken this session: its paired suite twice left the `:4886`
renderer unresponsive to the debugger for over a minute, which is noted rather than explained.

## Memory: re-installing must not retain the realm it replaces

Measured in Chrome for Testing 149 with `--js-flags=--expose-gc` (`Runtime.queryObjects` is
realm-blind, so WeakRefs from the page were used): 20 navigations of one same-origin child, then
`gc()` ×3 and `HeapProfiler.collectGarbage`.
...
## Timing of the thing being fixed
